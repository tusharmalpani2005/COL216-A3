        ...
        for (int c = 0; c < 4; c++) {
            ...
            if (idx >= 0 && C.sets[set][idx].state != I) {
                if (isWrite) {
                    ...
                    if (C.sets[set][idx].state == M) {
                        ...
                        if (bus.free_at(global_cycle)) {
                            ...
                            // First, collect all invalidations needed
                            for (int o = 0; o < 4; o++) {
                                if (o != c) {
                                    ...
                                    if (oi >= 0 && cache[o].sets[set][oi].state != I) {
                                        ...
                                        // Apply invalidations in the next cycle
                            ...
                            if (invalidated_others) {
                                // Use bus for 1 cycle for invalidations
                                st[c].invalidations++;
                            }
                ...
                if (isRdX) { // Write miss
                    ...
                    new_state = State::M;  // Corrected: Write miss goes directly to M state
                ...
                // Schedule invalidations for write miss in the next cycle
